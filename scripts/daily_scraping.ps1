...
for ($i = 0; $i -lt $RaceIds.Count; $i++) {
    ...
    if ($DryRun) {
        Write-Log "Dry Run: python main.py --race-id $RaceId --mode scrape_and_parse"
        $SuccessCount++
    ...
    try {
        $Process = Start-Process -FilePath "python" -ArgumentList "main.py --race-id $RaceId --mode scrape_and_parse" -Wait -PassThru -NoNewWindow
