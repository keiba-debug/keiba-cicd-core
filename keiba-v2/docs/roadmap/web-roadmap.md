...
### 概要
種牡馬/母父の統計データを分析・閲覧するページ。
`sire_stats_index.json` のデータを可視化。出遅れ分析ページと同じパターン。

### タブ構成案
1. **種牡馬ランキング**: 複勝率、勝率、出走数でソート可能なテーブル
2. **休み明け耐性ランキング**: fresh_advantage上位/下位のsire一覧（H3仮説）
3. **間隔詰め耐性ランキング**: tight_penalty上位/下位のsire一覧（H4仮説）
4. **母父（BMS）統計**: 同様の統計をBMS別で表示

### 実装方針
- **Backend**: `sire_stats_index.json` をそのままAPIから返す or 軽量に加工
- **Frontend**: 出遅れ分析ページ (`/analysis/slow-start`) のパターンを踏襲
- **フィルタ**: 最小出走数（10/30/50）、年齢層、距離帯
