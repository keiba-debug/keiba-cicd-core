...
|-------|--------|-----------|-----------|
| **★P** | **前走→今回 条件比較** | ⚠️ **最優先** | prev_finish, distance_change, track_type_change, condition適性 |
| **0** | 障害レース除外 | ✅ 完了(Session 64-65) | フィルタ + 障害専用モデル |
...
## ★ Phase P: 前走→今回 条件比較（最優先）

> Session 70で発見: past_features.pyは集約統計のみで「前走→今回」直接比較が一切ない。
> dam_top3_rateが重要度#1(290K)で支配的 = 「馬の一般的な強さ」は捕捉できているが
> 「今回のレース条件に合うか」が弱い。距離#39、馬場状態#106。
> 馬詳細ページでは距離別/馬場状態別/頭数別の成績を表示済み → 予測に活用されていない。

### 問題の構造

```
現状:  past_features.py → 集約統計（avg_finish_last3, distance_fitness等）
                          → 「前走」の情報が3走平均や累積率に薄まっている

必要:  「前走→今回」の条件変化を直接特徴量化
       → 能力（前走着順）× 適性（条件変化）× 調子（間隔・体重変化）
```
...
| 特徴量 | 型 | 値域 | ソース | 備考 |
|--------|-----|------|--------|------|
| `prev_finish` | int | 1-18 | history.finish_position | 前走着順（直接値） |
| `prev_distance` | int | 1000-3600 | history.distance | 前走距離 |
| `distance_change` | int | -2000〜+2000 | current - prev | 距離変化 |
| `prev_track_type` | int | 0/1 | history.track_type | 前走芝(0)/ダ(1) |
| `track_type_change` | int | -1/0/+1 | current - prev | 芝↔ダ転換 |
| `prev_track_condition` | int | 0-3 | **要追加** | 前走馬場状態(良0/稍1/重2/不3) |
| `condition_change` | int | -3〜+3 | current - prev | 馬場状態変化 |
| `prev_last3f` | float | 30-42 | history.last_3f | 前走上がり3F |
| `prev_odds` | float | 1-999 | history.odds | 前走オッズ（市場評価変化） |

### Tier 2 — 今回条件特化の適性率

馬詳細ページで既に表示されている集計を特徴量化:

| 特徴量 | 型 | ソース | 備考 |
|--------|-----|--------|------|
| `exact_distance_top3_rate` | float | history | 今回と同距離(±100m)での複勝率 |
| `same_condition_top3_rate` | float | history + **要cache追加** | 今回と同馬場状態での複勝率 |
| `same_venue_top3_rate` | float | history | = 既存venue_top3_rate（確認） |
| `field_size_top3_rate` | float | history | 同規模頭数(少/中/多)での複勝率 |
| `distance_category_match` | float | history | 短距離/マイル/中距離/長距離カテゴリの成績率 |

### Tier 3 — 前走パフォーマンス詳細

| 特徴量 | 型 | ソース | 備考 |
|--------|-----|--------|------|
| `prev_time_diff` | float | race_json.margin | 前走の勝ち馬とのタイム差 |
| `prev_entry_count` | int | history.num_runners | = 既存prev_race_entry_count |
| `prev_futan` | float | history.futan | 前走斤量 |
| `prev_horse_weight` | int | history.horse_weight | 前走馬体重 |

### 実装方針

1. **horse_history_cacheにtrack_conditionを追加** — build_horse_history.pyで取得
2. **past_features.pyにTier 1を追加** — 前走データを直接取り出す関数
3. **Tier 2は新モジュールまたはpast_features拡張** — 条件特化集計
4. **experiment.pyで検証** — 特徴量追加前後のAUC/VB ROI比較

### 事前加工の検討

| 方法 | メリット | デメリット |
|------|---------|-----------|
| predict時にリアルタイム計算 | 常に最新 | 計算コスト |
| 事前に馬別サマリJSON作成 | 高速、豊富な集計可能 | 更新タイミング管理 |

ユーザー提案: 事前加工（馬ごとの成績まとめ）でもOK。
馬詳細ページのHorseStatsSectionと同様の集計をMLパイプラインでも利用する設計が望ましい。
