...
## v8.4 / polaris 2.4: 学習期間アブレーション (2026-06-06, P3)

### 背景
前年同月(2025.06)を学習に入れると recency 効果があるかを α/β で検証。共通 OOS test = 2025.07〜2026.05。

### 分割
| 版 | train | val |
|----|-------|-----|
| 2.4a (α) | 2020〜2025-05 | 2025-06 |
| 2.4b (β) | 2020〜2025-06 | 2025-07 |

### 結果 (共通 test)
| 指標 | v2.3 | 2.4a | 2.4b |
|------|------|------|------|
| P AUC | 0.8003 | 0.8028 | 0.8028 |
| W AUC | 0.7907 | 0.7947 | 0.7939 |
| P Top1 ROI CI下限 | **73.8%** | 73.8% | 71.3% |
| W Top1 ROI | 83.5% | 80.0% | **83.2%** |

### 判定
- β>α on W Top1 ROI → 前年同月は W に効く
- ROI CI下限で v2.3 未超過 → **不採用・現状維持**
- live 切替なし (`--no-set-active` + `restore_live`)

### ⚠ 注記（2026-06-13 / Session 156 追記）: test期間が実質 2026-03 で切り詰め
本実験（6/6実行）は ML キャッシュ凍結インシデント（[ml-cache-staleness] `race_date_index.json` 3/17凍結・
`horse_history_cache.json` 3/15走まで）の**修復前**に走った。`experiment.py` は両キャッシュを直読みするため、
- 宣言上の test「2025-07〜2026-05」に対し、**実データは 2026-03 で途切れ**（eval_p3_v24a/b_result.json 実測:
  2026-04 = 0件、2026-05 = 0件）＝OOS が約2.5ヶ月分短い
- 2.4a/2.4b/v2.3 の3者は**同一の切り詰めtestで比較**しているため相対判断の公平性は保たれているが、
  CI下限ベースの採否判定はサンプル不足側に倒れている可能性がある
- **キャッシュ修復済み（6/12再構築、〜2026-06-07）の現在、フル窓での再アブレーションを推奨**。
  W Top1 ROI で β>α の好兆候が出ていた点も再判定対象

同様に、凍結期間（2026-03-17〜06-12）に実行された experiment 系 eval はすべて test 切り詰めの疑いあり
（v2.3 昇格判定 4/26 含む。同窓比較ゆえ相対判断は概ね成立、絶対値・CI は再計測が望ましい）。

### ✅ 再計測完了（2026-06-13, Session 157）: フル窓でも不採用＝v2.3 維持を確認
健全キャッシュでフル test 窓（2025.07〜2026.05 = **3,075R / 42,448行・全11ヶ月の実在を月別件数で確認**）を再計測。
v2.3 は archive から再評価（学習不要）、2.4a/2.4b は同一レシピで再学習（train は凍結の影響なし＝正当な再現）。
評価は全て `ml.evaluate_period`（較正後確率・Bootstrap 1000×）で統一。
...
- P Top1 ROI CI下限: 2.4a/2.4b とも 74.3 < v2.3 の 74.7 → **改善なし（旧判定どおり不採用・v2.3 維持で確定）**
- W Top1 は v2.3 が点推定でも最良（81.5%）。切り詰め窓で見えていた「前年同月は W に効く（β>α +3.2pt）」は
  フル窓で **+0.3pt に消滅＝切り詰めtestのアーティファクト**。「W only recency」案も根拠喪失
- 成果物: `eval_p3_{v23,v24a,v24b}_full_s157.json` / 2.4a・2.4b は `models/polaris/archive/` に今回は実体退避済み
- live は restore_live で v2.3 復元・archive とハッシュ一致確認済み

→ 詳細: `docs/ml-experiments/v8.4_polaris24_train_period_ablation.md`
