...
### Tier 1: 低コスト・高期待値
- [ ] **特徴量pruning**: importance下位20%を段階的に削除してVB ROI変化を測定
- [ ] **障害レース除外**: experiment.py / predict.py にフィルタ追加（5分で完了）
- [ ] **MARKET/VALUE自動分類テスト**: 特徴量1つずつModel Bに追加してA-B乖離への影響を測定
- [ ] **バックテストROI信頼区間**: Bootstrap法でROI推定の不確実性を定量化
...
### ハイパーパラメータ・アーキテクチャ
- [ ] Optunaによる体系的探索
- [ ] Cross-validation（時系列5-fold）でROI分散を測定
