...
### 障害馬単バックテスト（bet_engine実装）

**実装**: `bet_engine.py` — 3パターン（重複排除、最大3点/レース）

| パターン | 説明 | 1着モデル | 2着モデル |
|----------|------|-----------|-----------|
| P1→P2 | 複勝Top1→Top2 | Place | Place |
| W1→W2 | 勝率Top1→Top2 | Win | Win |
| P1→W2 | クロス（P1≠W1時のみ） | Place | Win |

**analyze_obstacle_multi バックテスト結果**（767レース、predictions.jsonベース）:

| 戦略 | 買い点 | 的中 | 的中率 |
|------|--------|------|--------|
| 馬単 W1→P2 | 767 | 67 | 8.7% |
| 馬単 W1→W2 | 767 | 70 | 9.1% |
| 馬単 W1→P2,3 (2点) | 1,534 | 121 | 7.9% |
| 馬単 W1→W2,3 (2点) | 1,534 | 123 | 8.0% |

> 注: 767レースの多くは旧障害モデル（W model未搭載）の予測。v2.5bモデルでは
> W Top1勝率が45%に向上しているため、W1ベースの馬単精度はさらに改善が見込まれる。

**W1が1着の時、2着馬の分布**（255回中）:
...
→ **W1が勝った時、P2-3が2着に来る確率は47.5%**。P1→P2 + W1→W2 + P1→W2 の
3点クロスで2着馬を高い確率でカバーできる。

**bankroll simulation (2025-03〜2026-03, 全券種混合)**:
- 馬単は全プリセット共通で **220bet、的中率8.6%**
- adaptive 5%プリセットで全体ROI +332.4%（馬単含む全券種）
