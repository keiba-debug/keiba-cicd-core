# -*- coding: utf-8 -*-
"""買い軸印 (markSet=8) の抽出 + DAT writer + 施錠/凍結ガード テスト。

設計: docs/auto-purchase/23_AI_MARK_VOTE_SYNC_DESIGN.md (案C)
カバレッジ:
  - ledger (raw_legs 積集合) → 軸(◆)+相手(◇) 抽出 (アンカー/単票/box/superseded)
  - ◆◇ の CP932 byte が TS target-mark-reader と対称 (819f/819e)
  - write_buy_marks_to_dat round-trip (write → read_marks_from_dat)
  - 施錠: 買い writer は markSet=1/6 拒否 (markSet=6 凍結ガード)、AI writer は markSet=8 拒否
  - markSet=8 へ書いても markSet=6 (AI評価) は不変
"""
...
def test_buy_mark_bytes_match_ts():
    """◆=0x819f / ◇=0x819e (target-mark-reader.ts の MARK_BYTES_TO_SYMBOL と一致)。"""
    assert dat_writer._SYMBOL_TO_MARK_BYTES["◆"] == b"\x81\x9f"
    assert dat_writer._SYMBOL_TO_MARK_BYTES["◇"] == b"\x81\x9e"
    # CP932 round-trip でも一致 (記号が壊れていない保証)
    assert "◆".encode("cp932") == b"\x81\x9f"
    assert "◇".encode("cp932") == b"\x81\x9e"
...
def test_buy_roundtrip(jv_root):
    ...
    n = dat_writer.write_buy_marks_to_dat(rid, {5: "◆", 7: "◇", 12: "◇"}, mark_set=8)
    assert n == 3
    got = dat_writer.read_marks_from_dat(rid, mark_set=8)
    assert got == {5: "◆", 7: "◇", 12: "◇"}
    assert (tmp / "MY_DATA" / "UmaMark8").exists()
...
def test_buy_clear_first(jv_root):
    ...
    _, _ = jv_root
    rid = "2026053105021204"
    dat_writer.write_buy_marks_to_dat(rid, {5: "◆", 7: "◇"}, mark_set=8)
    dat_writer.write_buy_marks_to_dat(rid, {3: "◆"}, mark_set=8)
    got = dat_writer.read_marks_from_dat(rid, mark_set=8)
    assert got == {3: "◆"}
...
def test_buy_writer_rejects_markset_6_frozen(jv_root):
    """買い軸 writer は markSet=6 (AI評価) を拒否 = 凍結ガード (条件①)。"""
    _, _ = jv_root
    with pytest.raises(ValueError, match="凍結"):
        dat_writer.write_buy_marks_to_dat("2026053105021204", {5: "◆"}, mark_set=6)


def test_ai_writer_rejects_markset_8(jv_root):
    """AI評価 writer は markSet=8 (買い軸) を拒否 = 相互施錠。"""
    _, _ = jv_root
    with pytest.raises(ValueError, match="買い軸印専用"):
        dat_writer.write_ai_marks_to_dat("2026053105021204", {5: "◎"}, mark_set=8)
...
def test_buy_writer_rejects_eval_marks(jv_root):
    ...
    with pytest.raises(ValueError, match="許可されない印"):
        dat_writer.write_buy_marks_to_dat("2026053105021204", {5: "◎"}, mark_set=8)


def test_buy_write_does_not_touch_markset6(jv_root):
    """markSet=8 への書込みは markSet=6 (AI評価) を一切変えない (物理スロット分離)。"""
    _, mm = jv_root
    rid = "2026053105021204"
    # markSet=6 に AI評価◎を書く
    dat_writer.write_ai_marks_to_dat(rid, {9: "◎"}, mark_set=6)
    # markSet=8 に買い軸を書く
    dat_writer.write_buy_marks_to_dat(rid, {5: "◆", 7: "◇"}, mark_set=8)
    # markSet=6 は不変
    m6 = mm.load_my_marks(rid, mark_set=6)
    assert m6[9].mark_symbol == "◎"
    assert set(m6.keys()) == {9}
    # markSet=8 は買い軸のみ
    m8 = dat_writer.read_marks_from_dat(rid, mark_set=8)
    assert m8 == {5: "◆", 7: "◇"}
