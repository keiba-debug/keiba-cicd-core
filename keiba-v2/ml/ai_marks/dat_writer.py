# -*- coding: utf-8 -*-
"""AI予想印を TARGET DAT (markSet=6) に書く Python writer。
...
安全機構 (シズネ施錠ガード):
  - mark_set=1 への書込みは例外 (ふくだ手動印専用)。AI印は 6 固定。
  - 書込み印は VALID_MARKS のみ (typo 無検証書込み防止)。
...
def write_ai_marks_to_dat(
    ...
    Args:
        ...
        mark_set: 既定 6 (AI印)。1 を渡すと例外 (施錠ガード)。
