...
def df_to_race_predictions(df_test: pd.DataFrame) -> list:
    """DataFrame → bet_engine 入力形式に変換

    bet_engine.generate_recommendations() は predict_race() と同じ
    entry dict 形式を期待するので、df_test を変換する。
    """
    races = []
...
def calc_bet_engine_roi(recs, race_predictions):
    """推奨買い目の実ROI計算
