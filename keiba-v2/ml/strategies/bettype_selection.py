...
#   ★0.85 に確定★ (実払戻 2 期間 sweep `sweep_combo_ev_floor.py` で ROI 最高・maxDD 最小):
#     P1 単のみ27%→15%・ROI 77.7→79.1%・DD 465k→434k / P2 24%→11%・80.4→80.7%・656k→630k。
#   ★0.70 は下げすぎ★: 券種別×EV帯ROIでは 0.70-0.85 帯が最良 (馬連103/三連複122/三連単102) に
#     見えるが、 それは「券種別 最良 1 plan」の数字。 floor を 0.70 に下げると EV 0.70-0.85 の plan が
#     ★複数束で選ばれ点数が増え、 当たらない点が増えて DD が膨張★ (両期間で ROI -0.6pt・DD微増)。
#     = 個別券種ROIに釣られると束効果を見落とす。 戦略全体の実払戻では 0.85 が最適点。
#   ★合成オッズ>単 (vs_tansho=='gt') の merit フィルタと AND なので「広げる価値ある combo だけ」を拾う。
#   ★「単のみ」レースの薄さは ★使い切り保証★ (size_race_fixed_grade の残余→単上乗せ) で別途解消済
#     (単のみでも cap=3000 まで単に上乗せ)。 floor と使い切りは別レイヤーの対策。
