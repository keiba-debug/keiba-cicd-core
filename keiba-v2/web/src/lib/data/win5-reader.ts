...
const FILENAME = 'win5_combo_results.json';

/**
 * WIN5シミュレーション結果を読み込む
 * version指定時はアーカイブ (versions/v{version}/) から読み込み
 */
export async function getWin5SimulationResult(version?: string | null): Promise<unknown | null> {
...
/**
 * win5_combo_results.json が存在するバージョン一覧を返す（新しい順）
 */
export async function getWin5SimulationVersions(): Promise<string[]> {
