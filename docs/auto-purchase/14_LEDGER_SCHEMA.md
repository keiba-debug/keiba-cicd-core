...
| `payout_source` | string | optional | **(Session 136)** settle 後の配当ソース。`"db"`（mykeibadb 確定配当）。的中だが DB 配当が取れない場合は payout を**捏造せず settle 自体を見送る**（元返しフォールバック廃止、シズネ 🔴-2）|
| `reconciled` | bool | optional | **(Session 136)** settle 後: IPAT 実投票履歴と突合（13章 reconcile）済か。`false`=暫定（pre-reconcile）。reconcile 本体実装までは常に `false`。表示層は `false` を「⚠未突合・暫定」と明示（シズネ 🔴-1）|
...
- `SETTLED` の payload に `portfolio_pnl` / `portfolio_roi` 追加（portfolio 単位の損益記録）。**(Session 136)** さらに `reconciled`（bool）/ `source`（`"db_payout_pre_reconcile"` / `"db_payout_reconciled"`）を追加 — settle が突合前の暫定か確定かを event レベルで識別（§12）。`ml/settle_ledger.py` が発火元（07 v1 の `settle_purchases` に加え）|
...
## 12. settle（精算）の前提条件と provenance（Session 136）

> **2026-05-30 追記**。`ml/settle_ledger.py` + `writer.record_settlement()` 実装に伴い、シズネ Session 136 レビュー 🔴-1/-2/-3 を構造化。**核心: 「不正確と分かっている値を、不正確と分からない形で税務 SoT に焼き込まない」**。

### 12.1 settle の状態と provenance フィールド

settle 時、各 ticket に次を書く（§2.1）:
...
SETTLED イベント（§6 No.12）payload に `source`（`"db_payout_pre_reconcile"` / `"db_payout_reconciled"`）と `reconciled` を刻む。**force 再 settle のたびに SETTLED イベントを追記**＝「なぜ・いつ再 settle したか」の監査証跡（改ざんでなく履歴）。

### 12.2 settle しない（SUBMITTED 据え置き）ケース
...
### 12.3 reconcile 前提条件（confirmed settle の定義）

settle は次の 2 段階を持つ:

| 段階 | 条件 | 記録 |
|---|---|---|
| **provisional settle**（暫定）| 着順確定 + DB 配当取得済。**ただし IPAT 突合は未実施** | `reconciled=false`。表示層で「⚠未突合・暫定」明示 |
| **confirmed settle**（確定）| 上記 + 13章 reconcile 実行済（severity WARN 以下）| `reconciled=true`。`--force` で provisional を上書き |

reconcile 本体（13章 IPAT CSV 突合）は**未実装**のため、現状の settle は**常に provisional**（`reconciled=false`）。`--allow-pre-reconcile` フラグは将来 reconcile を前提条件化したとき confirmed settle と区別するための意思表示ゲート（現状は挙動不変）。

**なぜ重要か（🔴-1）**: ledger は現状 `record_tansho_vote` で **bet_type=tansho 固定**かつ複数買い時に**過少記録**（Session 135 recorded=3/11）があり、**実 IPAT 投票と乖離**しうる。例: 2026-05-30 京都4R は ledger 上 tansho 馬番13（payout 320）だが、実 IPAT は馬単（≈1,090）。この乖離は**記録側（selection）の課題で次セッション対応**だが、その間も settle 値が「確定」に見えると税務 SoT に虚偽が残る。`reconciled=false` + 表示バッジで「これは未検証の暫定値」と SoT 自身に言わせることで、**「正しく見える嘘」を「暫定だと正直に言う記録」に変換**する。

### 12.4 運用 SOP（19_OOS_RUNBOOK 連携）

```
当日: 投票 → ledger SUBMITTED
夜〜翌朝: python -m ml.settle_ledger --date YYYY-MM-DD   # provisional settle (reconciled=false)
       → 収支管理/自動投票履歴で「⚠未突合・暫定」表示で確認
将来(reconcile実装後): IPAT CSV DL → reconcile → python -m ml.settle_ledger --date X --force  # confirmed settle (reconciled=true)
```
...
>
> （Session 136 追記）settle も同じ思想です。「確定に見える暫定値」を残さない。`reconciled` フラグ一つで、後で記録を直すまでの間も ledger は嘘をつかずに済みます。 — シズネ
