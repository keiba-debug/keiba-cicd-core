...
| イベント種類 | 13 種 | 13 + 5 (kill_switch/mode) + 4 (recon) = **22 種**（§6 で一元登録）|
...
## 6. ledger イベント正式登録（v1 13 種 + 12 章 5 種 + 13 章 4 種 = 22 種）

07 §5 で 13 種定義済。12 章 §4.6 / 13 章で追加された 9 種を v2 で正式登録する（前ターンで「次ターンで」と保留してた件のクローズ）。
...
| events | 1-13 (07 v1 全部) + `KILL_SWITCH_TRIGGERED` / `DAILY_OPT_IN` / `MODE_*` 系（既に 12 章で実装予定）|
