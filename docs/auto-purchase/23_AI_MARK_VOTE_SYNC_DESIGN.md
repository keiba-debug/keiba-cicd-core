...
## 7. 実装結果 + 将来「役割細分化」の前提 (2026-06-07)

**実装 (案C, markSet を 8→3 に付替=印スロット再編 26)**: `buy_marks.py`(ledger積集合→軸/相手)
+ `dat_writer.write_buy_marks_to_dat` + web `target-mark-reader` decode。記号は
**軸=★ / 相手=☆** (ふくだ確定。CP932 ★=819a / ☆=8199、既存印と非衝突)。markSet=3 = web「AI直前」列。

### ★☆ の意味 (現状 = 軸/相手の2値)
- **★ 軸** = portfolio 内 全 ticket の `raw_legs.horses` の **積集合** (アンカー型=軸1頭 / 単票=その馬)。
- **☆ 相手** = 和集合 − 軸 (流し先)。共通馬なし(box/formation)は軸なし=全☆。
- 出所 = ledger (実購入=**投票後**)。**未来レース(投票前)は ledger 無し=空**。
  投票前の「これから買う軸」を出すなら betting_selection.json 起点の別経路が要る (26 §4③ ギャップ)。

### ⚠️ 将来「頭固定 / 連軸 / 相手 / 3着まで」の役割細分化 (ふくだ問題提起 2026-06-07)
**現状の ledger では対応不可。記号でなく "データ" の問題。**
- 実査(6/7 ledger): `raw_legs` は **馬番集合のみ** (`{"horses":[3,10,5]}`)。
  **馬単・三連単ですら着順情報を持たない** (順序が積集合に潰れる)。
- → 「1着固定 / 連軸(着順不問) / 3着候補」を区別する情報源が存在しない。
- **対応の道筋 (3層)**:
  1. **データ層 (本丸)** = `bettype_scheduler`/選定 の出力に着順別 `raw_legs`
     (例 `{"1着":[3],"2着":[10,5],"3着":[10,5,9]}`) を持たせる。
  2. **抽出層** = `buy_marks` が券種×leg位置から役割判定 (頭固定/連軸/相手/3着)。
  3. **表示層** = 役割ごとに記号 (例 ★頭固定 / ◎連軸 / ▲相手 / △3着…)。
- markSet=3 の **1スロット内で記号を増設するだけで完結** (スロット追加不要。
  44byte/レコード・馬印2byte なので任意記号が入る)。設計上の行き止まりではない。
