...
> 📚 **書籍連携**: [書籍×プロジェクト実践学習ロードマップ](book_learning_roadmap.md) の Phase D で、FE実践（事例で学ぶ特徴量エンジニアリング）Ch 1-2（基本フレームワーク）、Ch 7（時系列特徴量）、Ch 4（バイアス検証）を keiba-v2 の8モジュールと紐づけて学べる。
...
| 特徴量が全て（004） | 特徴量エンジニアリング + クロス集計で仮説生成。FE実践 Ch 2,7 | Level 0-3, 3 |
...
| データはパワー（012） | データ品質管理、プロファイリング。FE実践 Ch 8（特徴量ストア） | Level DE-1 |
