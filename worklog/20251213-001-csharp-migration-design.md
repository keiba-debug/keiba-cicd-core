...
## 追記：機械学習設計書作成（2025-12-14 02:30）

### 作成ファイル

| ファイル | 内容 |
|---------|------|
| `docs/design/ml_training_pattern_analysis.md` | 調教パターン分析 機械学習設計書 |

### 分析目標

1. **コース別有効パターン**: 東京芝1600mなど特定条件で好走する調教パターン
2. **調教師別パターン**: 各調教師の得意な仕上げ方と結果の相関
3. **時系列パターン**: レース直前の調教強度と結果の関係
...
| テキスト分析 | TF-IDF / spaCy |
| 推論（本番） | ONNX Runtime (C#) |

### 実装フェーズ

1. Phase 1: EDA（1-2週間）
2. Phase 2: 特徴量エンジニアリング（2-3週間）
3. Phase 3: モデル構築（2-3週間）
4. Phase 4: パターン抽出（1-2週間）
5. Phase 5: C#統合（1-2週間）

### すぐに始められること

1. 既存の調教データ（CyokyoParser）と結果データ（SeisekiParser）を蓄積
2. ML.NET または Jupyter環境でEDA開始
3. 最低3ヶ月分のデータ収集を推奨
...
| 選択肢 | 推奨度 | 備考 |
|--------|--------|------|
| **ML.NET（推奨）** | ⭐⭐⭐ | C#で完結、LightGBMネイティブ対応 |
| **Azure AutoML** | ⭐⭐⭐ | ノーコード、自動最適化 |
| **Python** | ⭐⭐ | EDA・実験向け（オプション） |

**結論**: Pythonは必須ではない。ML.NETで学習・推論ともにC#で完結可能。
