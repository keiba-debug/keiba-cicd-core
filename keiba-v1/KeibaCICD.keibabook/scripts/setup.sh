...
if [ -f "src/keibabook/requirements.txt" ]; then
    log_info "requirements.txtから依存関係をインストール中..."
    if $PIP_CMD install -r src/keibabook/requirements.txt; then
...
log_info "1. .env ファイルに実際のCookieを設定してください"
    log_info "2. テストを実行してください: python src/keibabook/main.py --test"
    log_info "3. 実際のデータ取得を試してください: python src/keibabook/main.py --race-id [RACE_ID] --mode scrape_and_parse"
echo
